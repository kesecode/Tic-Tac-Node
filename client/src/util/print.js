...
const printInvitation = (isRevanche) => {
	...
	if (isRevanche) {
		...
		document.querySelector('#revancheAcc').addEventListener('click', onRevancheAccept);
		document.querySelector('#revancheDeni').addEventListener('click', onDeclineInvitation);
	} else {
		parent.appendChild(produceInvitationCard(false));
		document.querySelector('#revancheAcc').addEventListener('click', onPlayAgainAccept);
		document.querySelector('#revancheDeni').addEventListener('click', onDeclineInvitation);
	}
...
const printAwaitAcceptanceCard = (isRevanche) => {
	...
	if (isRevanche) {
		...
		document.querySelector('#revancheCancel').addEventListener('click', onCancelInvitation);
	} else {
		parent.appendChild(produceAwaitAcceptanceCard(false));
		document.querySelector('#playAgainCancel').addEventListener('click', onCancelInvitation);
	}
};
...
const printResultCard = (result) => {
	...
	if (!result) {
		document.querySelector('#revanche').addEventListener('click', onRevancheRequest);
	} else {
		document.querySelector('#playagain').addEventListener('click', onPlayAgainRequest);
	}
	scrollToBottom();
};
